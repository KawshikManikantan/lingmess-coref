...
class DocumentState(object):
    ...
    def finalize(self):
        ...
        merged_clusters = sorted([list(c) for c in merged_clusters])
...
def minimize_partition(name, language, dataset, extension, labels, stats):
    input_path = "{}.{}.{}.{}".format(name, dataset, language, extension)
    output_path = "{}.{}.{}.jsonlines".format(name,dataset,language)
    count = 0
        ...
        with open(output_path, "w") as output_file:
            ...
            for line in input_file.readlines():
                ...
                if document is not None:
                    document["clusters"] = [sorted(i) for i in document["clusters"]]
                    # for i in document["clusters"]:
                    #     print(i,sorted(i))
                    if name == "test":
                        print(document["doc_key"],document["clusters"])
...
def minimize_language(language,dataset, labels, stats):
    minimize_partition("dev", language, dataset, "v4_gold_conll", labels, stats)
    minimize_partition("train", language, dataset, "v4_gold_conll", labels, stats)
    minimize_partition("test", language, dataset, "v4_gold_conll", labels, stats)


if __name__ == "__main__":
    labels = collections.defaultdict(set)
    stats = collections.defaultdict(int)
    minimize_language("english","litbank", labels, stats)
